...
class GraphLearn(nn.Module):
    def __init__(self, input_dim, k_parameter, mode):
        ...
        self.adj_parameter = k_parameter
...
def init_model_dict(num_view, num_class, dim_list, dim_gcn_list, k_parameter, mode, gcn_dopout=0.5):
    model_dict = {}
    model_dict["FE"] = Encoder(dim_list, gcn_dopout)
    for i in range(num_view):
        model_dict['GL{:}'.format(i+1)] = GraphLearn(dim_list[i], k_parameter, mode)
        model_dict["GCN{:}".format(i+1)] = GCN_E(dim_list[i], dim_gcn_list, gcn_dopout)
        model_dict["C{:}".format(i+1)] = Classifier_1(dim_gcn_list[-1], num_class)
...
def init_optim(num_view, model_dict, lr_e, lr_c,reg):
    ...
    for i in range(num_view):
        optim_dict["C{:}".format(i+1)] = torch.optim.Adam(list(model_dict['GL{:}'.format(i+1)].parameters()) +
                list(model_dict["GCN{:}".format(i+1)].parameters())+list(model_dict["C{:}".format(i+1)].parameters()),
