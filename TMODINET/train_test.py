...
def train_epoch(data_list, adj_list, label, one_hot_label, sample_weight, model_dict, optim_dict, theta_smooth, theta_degree, theta_sparsity, neta,num_class,epoch,train_TCL=True):
    ...
    Encoder_list = model_dict["FE"](data_list)
    ...
    for i in range(num_view):
        ...
        ci = model_dict["C{:}".format(i+1)](model_dict["GCN{:}".format(i+1)](Encoder_list[i],normalized_adj))
    ...
    if train_TCL and num_view >= 2:
        ...
        for i in range(num_view):
            ...
            final_adj = neta * adj_train + (1 - neta) * adj_list[i]
            normalized_adj = normalize_adj(final_adj)
            GCN_list.append(model_dict["GCN{:}".format(i + 1)](Encoder_list[i],normalized_adj))

        tcl_data_list,loss_TCL= model_dict["TCL"](GCN_list)
        for i in range(num_view):
            ci_list.append(model_dict["C{:}".format(i + 1)](tcl_data_list[i]))
        ...
        c_loss = torch.mean(loss_un) + loss_CE + 0.8*loss_TCL
...
def test_epoch(data_list, adj_list, model_dict, neta):
    ...
    tcl_data_list = model_dict["TCL"](GCN_list,is_test=True)
    for i in range(num_view):
        ci_list.append(model_dict["C{:}".format(i + 1)](tcl_data_list[i]))
...
def train_test(data_folder, view_list, num_class,
    ...
    print("数据集：", data_folder)
    # 加载数据
    data_tr_list, data_te_list, trte_idx, labels_trte = prepare_trte_data(data_folder, view_list)
    print("训练样本个数：",data_tr_list[0].shape[0])
    print("测试样本个数：", data_te_list[0].shape[0])
    ...
    # 构造相似网络图
    adj_tr_list, adj_te_list = gen_trte_adj_mat(data_tr_list, data_te_list, k_parameter)
    ...
    model_dict = init_model_dict(num_view, num_class, dim_list, dim_gcn_list, k_parameter, mode)
